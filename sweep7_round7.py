"""Round 7: Survival-first - agent must survive long enough to profit."""
import json, subprocess, sys, time, random, re
from pathlib import Path

PYTHON = sys.executable
WORKER = str(Path(__file__).parent / "train_ui" / "worker.py")
OUTPUT_DIR = Path(__file__).parent / "sweep7_results"
...
CONFIGS = [
    {"name": "R7_0_survival", "desc": "surv_coeff=0.1, game_over=5, death=1, debt=0.005",
     "survival_coeff": 0.1, "game_over_penalty": 5.0, "death_penalty": 1.0,
     "debt_coeff": 0.005},
    {"name": "R7_1_nw_heavy", "desc": "surv_coeff=0.2, game_over=3, death=0, debt=0.003",
     "survival_coeff": 0.2, "game_over_penalty": 3.0, "death_penalty": 0.0,
     "debt_coeff": 0.003},
    {"name": "R7_2_nw_max", "desc": "surv_coeff=0.5, game_over=1, death=0, debt=0.001",
     "survival_coeff": 0.5, "game_over_penalty": 1.0, "death_penalty": 0.0,
     "debt_coeff": 0.001},
    {"name": "R7_3_s2_nw", "desc": "stage=2, surv_coeff=0.2, inc=15, born=10",
     "curriculum_stage": 2, "survival_coeff": 0.2, "daily_income": 15.0,
     "born_bonus": 10.0, "game_over_penalty": 3.0, "death_penalty": 0.0,
     "debt_coeff": 0.003},
]
...
def main():
    ...
    print(f"\n{'='*60}\nROUND 7 RESULTS\n{'='*60}")
    print(f"{'Name':<22} {'Score':>8} {'Bases':>6} {'Return':>10} {'People':>7} {'Days':>6}")
    print("-" * 70)
    for r in all_results:
        print(f"{r['name']:<22} {r.get('eval_score','?'):>8} {r.get('eval_bases','?'):>6} "
              f"{r.get('eval_return','?'):>10} {r.get('eval_people','?'):>7} {r.get('eval_days','?'):>6}")
    ...
    if valid:
        ...
        print(f"\nClosest: {best['name']}: return={best.get('eval_return')} score={best.get('eval_score')} bases={best.get('eval_bases')} days={best.get('eval_days')}")
